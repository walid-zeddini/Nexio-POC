...
import com.nexio.api.ms.config.KafkaProperties;
...
class NexioKafkaResourceIT {
    ...
    void setup() {
        ...
        NexioKafkaResource kafkaResource = new NexioKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/nexio-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/nexio-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
